...
class Feed
{
	...
	public function update_header_cache()
	{
		...
		if (!empty($remote_header_data) && isset($remote_header_data[0]) && isset($remote_header_data[0]['info']) ) {
			...
			if( ! SBR_Feed_Saver_Manager::check_api_limit( $request['provider'] ) ){
				...
				if (isset($new_data['data'])) {
					if ( ! empty( $new_data['data']['error'] ) ) {
						...
						$message .= '<br><br>';
						$this->add_error( $message, sprintf( __( 'Troubleshoot by visiting %serror message reference page%s.', 'reviews-feed' ), '<a href="https://smashballoon.com/docs/reviews-error-message-reference/?reviews&utm_campaign=reviews-pro&utm_source=feed&utm_medium=apierror&utm_content=Error%20Message%20Reference" target="_blank" rel="noopener noreferrer">', '</a>' ) );
					}
	...
	public function filter_posts($posts, $settings, $moderatePosts = false )
	{
		...
		foreach ($posts as $post) {
            ...
            if( isset( $post['provider'] ) && $post['provider']['name'] === 'facebook' ){
			...
			if ($passes_star_filter === true && $passes_word_filter && $passes_moderation) {
				$keep_post = true;
			}
