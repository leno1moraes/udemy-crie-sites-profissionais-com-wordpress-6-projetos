...
function sbr_plugin_settings_defaults(){
	return [
        ...
        'translations' => [
            'second' => __('second', 'sb-customizer'),
            'seconds' => __('seconds', 'sb-customizer'),
            'minute' => __('minute', 'sb-customizer'),
            'minutes' => __('minutes', 'sb-customizer'),
            'hour' => __('hour', 'sb-customizer'),
            'hours' => __('hours', 'sb-customizer'),
            'day' => __('day', 'sb-customizer'),
            'days' => __('days', 'sb-customizer'),
            'week' => __('week', 'sb-customizer'),
            'weeks' => __('weeks', 'sb-customizer'),
            'month' => __('month', 'sb-customizer'),
            'months' => __('months', 'sb-customizer'),
            'year' => __('year', 'sb-customizer'),
            'years' => __('year', 'sb-customizer'),
            'ago' => __('ago', 'sb-customizer'),
        ]
	];
}
...
function sbr_get_fb_connection_urls( $is_settings = false ) {
	$urls = array();
	$admin_url_state = ($is_settings) ? admin_url('admin.php?page=sbr-settings') : admin_url('admin.php?page=sbr');
	//If the admin_url isn't returned correctly then use a fallback
	if ($admin_url_state == '/wp-admin/admin.php?page=sbr' || $admin_url_state == '/wp-admin/admin.php?page=sbr&tab=configuration') {
		$admin_url_state = esc_url( "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]" );
	}
	$sb_admin_email = get_option('admin_email');
	$urls['page']  = 'https://connect.smashballoon.com/auth/fb/?wordpress_user=' . $sb_admin_email . '&vn=' . SBRVER . '&v=pro&prod=reviews&state=';
...
function sbr_plugin_action_links( $links ){
	...
	$links = array_merge(
		array(
			'<a href="' . esc_url( $settings_link ) . '">' . __('Settings', 'sb-reviews') . '</a>'
		), $links);

    if( !Util::sbr_is_pro() ){
        $links = array_merge(
            array(
                '<a href="https://smashballoon.com/instagram-feed/?utm_campaign=reviews-free&utm_source=plugins-page&utm_medium=upgrade-link&utm_content=UpgradeToPro" target="_blank" style="font-weight:bold; color: #50a56d;">' . __('Upgrade to Pro', 'sb-reviews') . '</a>'
            ),
            $links
        );
    }else{
        $links = array_merge(
            array(
                '<a href="' . esc_url($support_link) . '">' . __('Support', 'sb-reviews') . '</a>'
            ),
            $links
        );
    }
...
{
    load_plugin_textdomain('reviews-feed', false, basename(dirname(__FILE__)) . '/languages');
}
add_action('plugins_loaded', 'sbr_text_domain');
