...
class Util {
	...
	public static function get_channel_id_by_api_request( $url ) {
		...
		self::save_scraped_channel_id( $url, $response );

		return $response->channel_id;
	}

	/**
	 * Save scraped channel ID to database
	 * 
	 * @return void
	 */
	public static function save_scraped_channel_id( $url, $response ) {
		$channel_ids = get_option( 'sby_scraped_channel_ids' );
		...
		update_option( 'sby_scraped_channel_ids', json_encode( $channel_ids ) );
	}

	/**
	 * Get channel ID from scraped channel IDs saved in the database
	 * 
	 * @return null|string
	 */
	public static function get_scraped_channel_id( $channel ) {
		$channel_ids = get_option( 'sby_scraped_channel_ids' );
		$channel_ids = json_decode( $channel_ids, true );
		if ( empty( $channel_ids ) ) {
			return;
