...
use SmashBalloon\YouTubeFeed\Pro\SBY_API_Connect;
...
class MiscService extends ServiceProvider {
	...
	public function sby_do_import_batch() {
		...
		if ( empty( $playlist ) ) {
			$connection = new SBY_API_Connect( sby_get_first_connected_account(), 'channels', $params );
			$connection->connect();
		...
		if ( $playlist ) {
			...
			$playlist_connection = new SBY_API_Connect( sby_get_first_connected_account(), 'playlistItems', $params );
